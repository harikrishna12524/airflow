...
class GoogleCloudSecretManagerHook(GoogleBaseHook):
    ...
    def __init__(self, location_id: str | None = None, **kwargs) -> None:
    ...
    def client(self):
        ...
        if self.location_id is not None:
            return SecretManagerServiceClient(
                ...
                client_options={"api_endpoint": f"secretmanager.{self.location_id}.rep.googleapis.com"},
            )
        return SecretManagerServiceClient(
            credentials=self.get_credentials(),
            client_info=CLIENT_INFO,
    ...
    def _get_parent(self, project_id: str, location_id: str | None = None) -> str:
        """
        Builds and parent path. Used internally.

        :param project_id: Required. ID of the GCP project that owns the job.
        :param location_id: Optional. Target location ID.  If set to ``None`` or missing, the location_id provided for GoogleCloudSecretHook instantiation is used
            For more details : https://cloud.google.com/secret-manager/docs/locations

        :return: Parent path.
        """
        _location_id = location_id or self.location_id
        if _location_id is not None:
            return self.client.common_location_path(project_id, _location_id)
        return self.client.common_project_path(project_id)

    def _get_secret_path(self, project_id: str, secret_id: str, location_id: str | None = None) -> str:
        """
        Builds and returns the secret path. Used internally.
        ...
        if _location_id is not None:
            ...
            return f"projects/{project_id}/locations/{_location_id}/secrets/{secret_id}"
        return self.client.secret_path(project_id, secret_id)

    def _get_secret_version_path(
        self, project_id: str, secret_id: str, secret_version: str, location_id: str | None = None
    ) -> str:
        """
        Builds and returns the secret version path. Used internally.
        ...
        if _location_id is not None:
            ...
            return f"projects/{project_id}/locations/{_location_id}/secrets/{secret_id}/versions/{secret_version}"
    ...
    def create_secret(
        ...
        location_id: str | None = None,  # I have added at end for maintaining backward compatibility.
    ) -> Secret:
        """
        Create a secret.
        ...
        :param location_id: Optional. Location where secret should be created. Used for creating regional secret.  If set to ``None`` or missing, the location_id provided for GoogleCloudSecretHook instantiation is used
            For more details : https://cloud.google.com/secret-manager/docs/locations
        :return: Secret object.
        """
        if not secret:
            _secret: dict | Secret = {}
            if (location_id or self.location_id) is None:
        ...
        response = self.client.create_secret(
            request={
                "parent": self._get_parent(project_id, location_id),
                "secret_id": secret_id,
                "secret": _secret,
            },
            retry=retry,
    ...
    def add_secret_version(
        ...
        :param timeout: Optional. The timeout for this request.
        :param metadata: Optional. Strings which should be sent along with the request as metadata.
        :param location_id: Optional. Location where secret is located. Used for adding version to regional secret.  If set to ``None`` or missing, the location_id provided for GoogleCloudSecretHook instantiation is used
            For more details : https://cloud.google.com/secret-manager/docs/locations
        :return: Secret version object.
        """
        response = self.client.add_secret_version(
            request={
                "parent": self._get_secret_path(project_id, secret_id, location_id),
    ...
    def list_secrets(
        ...
        :param timeout: Optional. The timeout for this request.
        :param metadata: Optional. Strings which should be sent along with the request as metadata.
        :param location_id: Optional. The regional secrets stored in the provided location_id will be listed.  If set to ``None`` or missing, the location_id provided for GoogleCloudSecretHook instantiation is used
            For more details : https://cloud.google.com/secret-manager/docs/locations
    ...
    @GoogleBaseHook.fallback_to_default_project_id
    def secret_exists(self, project_id: str, secret_id: str, location_id: str | None = None) -> bool:
        """
        ...
        :param location_id: Optional. Location ID where secret is expected to be stored regionally.  If set to ``None`` or missing, the location_id provided for GoogleCloudSecretHook instantiation is used
            For more details : https://cloud.google.com/secret-manager/docs/locations
        :return: True if the secret exists, False otherwise.
        """
        secret_filter = f"name:{secret_id}"
        secret_name = self._get_secret_path(project_id, secret_id, location_id)
        for secret in self.list_secrets(
            project_id=project_id, page_size=100, secret_filter=secret_filter, location_id=location_id
        ):
    ...
    def access_secret(
        ...
        :param timeout: Optional. The timeout for this request.
        :param metadata: Optional. Strings which should be sent along with the request as metadata.
        :param location_id: Optional. Location ID where secret is stored regionally.  If set to ``None`` or missing, the location_id provided for GoogleCloudSecretHook instantiation is used
            For more details : https://cloud.google.com/secret-manager/docs/locations
        :return: Access secret version response object.
        """
        response = self.client.access_secret_version(
            request={
                "name": self._get_secret_version_path(project_id, secret_id, secret_version, location_id),
            },
            retry=retry,
            timeout=timeout,
            metadata=metadata,
    ...
    def delete_secret(
        ...
        :return: Access secret version response object.
        """
        name = self._get_secret_path(project_id, secret_id, location_id)
